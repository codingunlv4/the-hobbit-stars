...
func _ready() -> void:
	...
	$HBox/PreviewPanel/Margin/VBox/SubViewportContainer.visible = false
	$HBox/PreviewPanel/Margin/VBox/PreviewLabel.text = "Hero preview (3D off for stability)"
	$HBox/PreviewPanel.clip_contents = true
	$HBox/OptionsPanel.clip_contents = true
	$HBox/PreviewPanel/Margin/VBox/SubViewportContainer.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func _fix_ui_layer() -> void:
	# Put all clickable UI on a CanvasLayer so nothing blocks input/rendering.
	...
	# Full-screen dark backdrop stays behind UI; must not eat clicks.
	$Background.mouse_filter = Control.MOUSE_FILTER_IGNORE
	title.mouse_filter = Control.MOUSE_FILTER_IGNORE
	$HBox/PreviewPanel/Margin/VBox/SubViewportContainer.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func _process(delta: float) -> void:
	if not $HBox/PreviewPanel/Margin/VBox/SubViewportContainer.visible:
		return
...
func _refresh_preview(full := true) -> void:
	if full and $HBox/PreviewPanel/Margin/VBox/SubViewportContainer.visible:
...
func _refresh_partner_preview() -> void:
	if _selected_partner_id.is_empty() or not $HBox/PreviewPanel/Margin/VBox/SubViewportContainer.visible:
